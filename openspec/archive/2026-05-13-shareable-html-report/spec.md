# Spec (delta): `report` — Shareable HTML Report Extensions

**Change**: `shareable-html-report` (Issue #2 Block F)
**Modifies capability**: `report` (existing — main spec at `openspec/specs/report/spec.md` if present, else implied by `ReportGenerator.kt`)
...
- **WHEN** rendered
- **THEN** the output SHALL equal byte-for-byte the v4.5 output for the same non-KPI inputs (asserted by `KpiSectionsOffByDefaultTest` against a stored golden file)
