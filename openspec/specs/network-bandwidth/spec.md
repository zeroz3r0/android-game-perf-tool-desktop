# Spec: Network Bandwidth — Per-Tick Total-App RX/TX (Android)

This spec defines the per-tick total-app network bandwidth capability for Android capture sessions. Closes Issue #1 Gap 2 (GameBench parity roadmap, engram `roadmap/gamebench-parity` #289) at ~60% of the gap. It is a NEW capability — no prior `network-bandwidth` spec exists. The implementation reads cumulative byte counters via Android binder service call `netstats` (catalog-of-candidates probe) with `dumpsys netstats detail --uid` as fallback, computes per-tick deltas, and surfaces unavailability with a Spanish (tuteo-formal) diagnostic banner. See `archive/2026-05-13-network-bandwidth-total-app/proposal.md` and `design.md` for context, GameBench positioning, and risk analysis.
...
Honest positioning constraint: we read total-app cumulative byte counters only. We do NOT promise per-connection breakdown (`api.unity3d.com:443`), TTFB per endpoint, or per-interface wifi/cellular split — those would require eBPF or libc hook. HINT confidence until lab-verified across the OEM matrix.
...
`AdbBridge.captureNetworkBandwidth(deviceId, pkg, uid): NetworkSnapshot` MUST perform full probe (resolve UID + walk binder + maybe dumpsys) ONLY on first call per device per session, then cache the winning method. Subsequent calls MUST issue at most 1 shell command.
