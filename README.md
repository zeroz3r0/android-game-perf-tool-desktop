...
- **Detección automática de fases del juego (v4.6.0, Unity + Unreal)**: GamePerf identifica solo cuándo el juego está en cinemática, menú, combate, oleadas o tutorial — leyendo los nombres de escena que Unity y Unreal emiten en logcat por defecto. **Sin tocar el código del juego**, sin SDK, sin nada. Funciona con cualquier APK debuggeable. Reconoce keywords bilingües (español + inglés): `combat/combate/fight/boss/oleada` → COMBAT_PHASE; `cinematic/cinemática/cutscene/intro` → CUTSCENE; `tutorial/onboarding` → TUTORIAL_PHASE; `menu/lobby/inicio` → MENU_NAV. Las fases aparecen como eventos en el reporte para drill-down. Las cinemáticas se excluyen automáticamente de las medias de FPS del gameplay (igual que los anuncios); menús, combate y tutoriales NO se excluyen (son gameplay normal). Si los nombres del juego están ofuscados (por ejemplo `s001`, `level_42`), la clasificación no aplica y solo se emite LOADING. Cobertura v1: Unity + Unreal (Cocos2d/Godot/GameMaker pendientes)
- **Ancho de banda de red (v4.6.0, Android)**: cierra el segundo gap del roadmap «GameBench parity». Mide el total de bytes RX (descarga) y TX (subida) que consume el juego durante la captura. Lectura vía `service call netstats` (binder, rápido) con fallback automático a `dumpsys netstats detail --uid` cuando el binder no responde — sin root, sin SDK embebido. GameBench separa el tráfico por endpoint (api.unity3d.com:443 separado de api.facebook.com:443) usando hook libc o eBPF, lo cual requiere instrumentar el juego; nosotros medimos el total agregado de la app, que es lo que suele importar para detectar fugas de bandwidth, anuncios pesados o uploads no planeados. En dispositivos donde no podemos leer (Android viejo sin binder, dumpsys con permisos denegados), el reporte muestra un banner en castellano con el motivo
- **Detecta wake locks excesivos con pantalla apagada (v4.6.0, criterio Google Play Vitals 2024)**: GamePerf mide cuánto tiempo el juego mantiene la CPU activa con la pantalla apagada y te avisa si cruza el gate de Google (más de 2 h acumuladas). Google Play penaliza esto con throttling de descubrimiento de la app en la tienda — la causa típica es un SDK de analíticas mal configurado (Firebase Analytics, AppsFlyer, Adjust, Branch, Crashlytics) que hace jobs frecuentes en background. Lectura vía `dumpsys batterystats` por adb, sin root, sin SDK embebido. El reporte añade una sección «Wake locks (pantalla apagada)» con el total acumulado en horas, la cita oficial de Vitals (< 5 % sesiones cruzando > 2 h en 24 h) y, si cruzas el gate, un banner diagnóstico con las posibles causas. GamePerf también reporta proxies single-session de los otros dos KPIs Vitals store-gating: Crash Rate Users (< 1.09 %) y ANR Rate Users (< 0.47 %)
...
- **Bandas RAG por KPI y líneas de presupuesto en los gráficos (v4.7.0)**: cada KPI muestra una banda verde / ámbar / rojo con indicador accesible (forma + texto, no solo color) según el tier de dispositivo detectado, y los charts de tiempo por frame incluyen líneas de presupuesto a 60 fps (16.6 ms) y 30 fps (33.3 ms), más 120 fps (8.3 ms) en flagships. Los umbrales vienen del catálogo central anclado en Google Play Vitals 2024 y PerfDog publicado, no son arbitrarios
