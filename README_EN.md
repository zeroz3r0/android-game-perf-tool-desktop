...
- **Automatic game phase detection (v4.6.0, Unity + Unreal)**: GamePerf detects on its own when the game is in a cutscene, menu, combat phase, wave, or tutorial — by reading the scene names that Unity and Unreal emit in logcat by default. **Without touching the game's source code**, no SDK, no instrumentation. Works with any debuggable APK. Recognizes bilingual keywords (English + Spanish): `combat/combate/fight/boss/wave` → COMBAT_PHASE; `cinematic/cinemática/cutscene/intro` → CUTSCENE; `tutorial/onboarding` → TUTORIAL_PHASE; `menu/lobby/home` → MENU_NAV. Phases appear as events in the report for drill-down. Cutscenes are automatically excluded from gameplay FPS averages (same pattern as ads); menus, combat and tutorials are NOT excluded (they are normal gameplay). If the game's scene names are obfuscated (e.g. `s001`, `level_42`), classification doesn't apply and only LOADING is emitted. v1 coverage: Unity + Unreal (Cocos2d/Godot/GameMaker not supported yet)
...
- **Detects excessive wake locks with screen off (v4.6.0, Google Play Vitals 2024 criterion)**: GamePerf measures how long the game keeps the CPU awake with the screen off and warns you if it crosses Google's gate (more than 2 h accumulated). Google Play penalizes this with discovery throttling on the store — the typical cause is a misconfigured analytics SDK (Firebase Analytics, AppsFlyer, Adjust, Branch, Crashlytics) running frequent background jobs. Read via `dumpsys batterystats` over adb, no root, no embedded SDK. The report adds a "Wake locks (screen off)" section with the accumulated total in hours, the official Vitals citation (< 5% of sessions crossing > 2 h in 24 h) and, if you cross the gate, a diagnostic banner with the likely causes. GamePerf also reports single-session proxies for the other two store-gating Vitals KPIs: Crash Rate Users (< 1.09%) and ANR Rate Users (< 0.47%)
...
- **RAG bands per KPI and budget lines on the charts (v4.7.0)**: every KPI shows a green / amber / red band with an accessible indicator (shape + text, not color alone) based on the detected device tier, and frame-time charts include budget reference lines at 60 fps (16.6 ms) and 30 fps (33.3 ms), plus 120 fps (8.3 ms) on flagships. Thresholds come from the central catalog anchored on Google Play Vitals 2024 and published PerfDog formulas, not arbitrary
